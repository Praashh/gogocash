...
const products: Product[] = [
  {
    ...
    image: "https://images.unsplash.com/photo-1518442072849-5e30d89848b8?q=80&w=1200&auto=format&fit=crop",
  },
  {
    id: "2",
  ...
  },
  {
    ...
    inStock: true,
    category: "Electronics",
    image: "https://images.unsplash.com/photo-1490376840453-5f616fbebe5b?q=80&w=1200&auto=format&fit=crop",
  },
]

type SortKey = "relevance" | "best" | "price-asc" | "price-desc" | "commission-desc"
...
export default function Page() {
  ...
  const categories = React.useMemo(() => ["All", ...Array.from(new Set(products.map((p) => p.category)))], [])
  const sellers = React.useMemo(() => ["All", ...Array.from(new Set(products.map((p) => p.sellerType)))], [])
  const countries = React.useMemo(() => ["All", ...Array.from(new Set(products.map((p) => p.country)))], [])
  ...
  const filtered = React.useMemo(() => {
    return products
      ...
      .filter((p) => (minCommission ? p.commissionRate * 100 >= Number(minCommission) : true))
      .filter((p) => (inStock ? p.inStock : true))
      .filter((p) => (prime ? p.prime : true))
  }, [query, category, seller, country, minPrice, maxPrice, minCommission, inStock, prime])
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          ...
          <Input inputMode="numeric" value={minPrice} onChange={(e) => setMinPrice(e.target.value)} placeholder="0" className="bg-transparent text-white border-white/20" />
        </div>
        <div className="space-y-1">
          <label className="text-xs text-white/70">Max Price</label>
          <Input inputMode="numeric" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} placeholder="1000" className="bg-transparent text-white border-white/20" />
        </div>
      </div>
      <div className="space-y-1">
        <label className="text-xs text-white/70">Min Commission %</label>
        <Input inputMode="numeric" value={minCommission} onChange={(e) => setMinCommission(e.target.value)} placeholder="0" className="bg-transparent text-white border-white/20" />
      </div>
      <div className="flex items-center gap-2">
        <Checkbox id="stock" checked={inStock} onCheckedChange={(v) => setInStock(Boolean(v))} />
        <label htmlFor="stock" className="text-sm text-white/80">In stock</label>
      </div>
      <div className="flex items-center gap-2">
        <Checkbox id="prime" checked={prime} onCheckedChange={(v) => setPrime(Boolean(v))} />
        <label htmlFor="prime" className="text-sm text-white/80">Fast shipping</label>
      </div>
      <Button variant="secondary" className="w-full" onClick={() => { setCategory("All"); setSeller("All"); setCountry("All"); setMinPrice(""); setMaxPrice(""); setMinCommission(""); setInStock(false); setPrime(false) }}>Reset</Button>
    </div>
  )
    ...
    <div className="min-h-screen bg-black text-white relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-10">
        <div className="mb-6 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <Input placeholder="Search products" value={query} onChange={(e) => setQuery(e.target.value)} className="w-56 bg-transparent text-white border-white/20" />
            ...
            <div className="sm:hidden">
              <Sheet>
                ...
                <SheetContent side="right" className="w-80 bg-black text-white border-white/20">
                  <SheetHeader>
          ...
          <main>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {sorted.map((p) => {
                ...
                return (
                  <Card key={p.id} className="bg-background/10 backdrop-blur border-white/10 overflow-hidden">
                    <div className="relative aspect-[4/3] bg-white/5">
                      <img src={p.image} alt={p.title} className="h-full w-full object-cover" />
                      {p.prime && (
                        <div className="absolute top-2 left-2">
                          <Badge className="bg-blue-500 text-black">Fast shipping</Badge>
                        </div>
                      )}
                    </div>
                    <CardHeader className="p-4 pb-2">
                      <CardTitle className="text-base text-white line-clamp-2 min-h-[2.75rem]">{p.title}</CardTitle>
                    </CardHeader>
                    <CardContent className="p-4 pt-0">
                      <div className="flex items-center gap-2">
                        <Stars value={p.rating} />
                        <span className="text-xs text-white/60">({p.reviews.toLocaleString()})</span>
                      </div>
                      <div className="mt-2 text-lg font-semibold">{formatMoney(p.price, p.currency)}</div>
                      <div className="mt-1 text-sm text-white/70">{p.sellerType} • {p.country}</div>
                      <div className="mt-3 flex items-center gap-2">
                        {isHigh && <Badge className="bg-teal-500 text-black">{Math.round(p.commissionRate * 100)}% commission</Badge>}
                        {!isHigh && <Badge variant="secondary" className="text-white/90">{Math.round(p.commissionRate * 100)}% commission</Badge>}
                        {!p.inStock && <Badge className="bg-red-500">Out of stock</Badge>}
                      </div>
              ...
              {sorted.length === 0 && (
                <div className="col-span-full text-white/70">No products match your filters.</div>
              )}
            </div>
          </main>
        </div>
